...
```sh
ai-on       # AI稼働モードをON（バッテリー駆動中は確認あり。--force で省略）
...
全体ショートカットの既定は `Option + Command + X`（`⌥⌘X`）です。カット（`⌘X`）などのシステム標準操作や、主要アプリの既定とぶつかりにくい組み合わせを選んでいます。押すたびにONとOFFがその場で切り替わり、結果は毎回、画面上部に一瞬だけ表示されます（どの画面・フルスクリーン中でも埋もれずに表示されます）。バッテリー駆動中にONへ切り替えた場合は、発熱と電池残量への注意もあわせて表示します。OFFはいつでも確認なしで、その場で通常のスリープに戻ります。ショートカットはメニューバーの `ショートカットを変更…` から、好きなキーの組み合わせに変更できます。Caps LockはmacOSで通常の修飾キーとして扱われず、文字入力中の誤操作も起きやすいため使用していません。

初回起動時にユーザー用のログイン起動へ登録します。メニューの `ログイン時にも起動` からいつでも解除できます。

設定はメニューバーの「設定…」（⌘,）か、操作画面右上の歯車から。ショートカットの変更・ログイン時の起動・パスワード省略をまとめてあります。
...
- バッテリー駆動中のONは、アプリでは確認ダイアログ、CLIでは対話確認または `--force` を通します。OFFはいつでも実行できます。
