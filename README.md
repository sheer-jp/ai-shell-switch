...
全体ショートカットは `Control + Option + A`（`⌃⌥A`）です。ONとOFFをその場で切り替えられます。バッテリー駆動中にONへ切り替えるときは、画面上部に案内が表示され、6秒以内にもう一度 `⌃⌥A` を押すと確定します（どの画面・フルスクリーン中でも埋もれずに表示されます）。アプリの画面やメニューから操作した場合は確認ダイアログをはさみます。OFFはいつでも確認なしで、すぐに効きます。Caps LockはmacOSで通常の修飾キーとして扱われず、文字入力中の誤操作も起きやすいため使用していません。
