...
全体ショートカットの既定は `Control + Option + Command + A`（`⌃⌥⌘A`）です。修飾キー3つの組み合わせは他アプリの既定とぶつかりにくいものを選んでいます。押すたびにONとOFFがその場で切り替わり、結果は毎回、画面上部に一瞬だけ表示されます（どの画面・フルスクリーン中でも埋もれずに表示されます）。バッテリー駆動中にONへ切り替えた場合は、発熱と電池残量への注意もあわせて表示します。OFFはいつでも確認なしで、その場で通常のスリープに戻ります。ショートカットはメニューバーの `ショートカットを変更…` から、好きなキーの組み合わせに変更できます。Caps LockはmacOSで通常の修飾キーとして扱われず、文字入力中の誤操作も起きやすいため使用していません。
