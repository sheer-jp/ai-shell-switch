...
全体ショートカットは `Control + Option + A`（`⌃⌥A`）です。ONとOFFをその場で切り替えられます。バッテリー駆動中にONへ切り替えるときだけ、電池の消耗と発熱を伝える確認ダイアログをはさみます。OFFはいつでも確認なしで、すぐに効きます。Caps LockはmacOSで通常の修飾キーとして扱われず、文字入力中の誤操作も起きやすいため使用していません。
